...
void solve(int n, std::vector<int>& dist, std::vector<int>& origin,
           std::vector<std::vector<int>>& matrix, int start, int finish) {
  std::set<int> visited;
  int target = start;
  dist[target] = 0;
...
int main() {
  ...
  std::vector<int> dist(n, -1), origin(n, -1);
  ...
  solve(n, dist, origin, matrix, --start, --finish);
